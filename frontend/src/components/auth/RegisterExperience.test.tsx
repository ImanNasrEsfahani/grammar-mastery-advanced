...
test("requires confirmation and terms before sending registration", async () => {
  ...
  await user.type(screen.getByPlaceholderText("Create a password"), "ExamplePassword1!");
  await user.type(screen.getByPlaceholderText("Re-enter your password"), "DifferentPassword1!");
...
test("toggles password visibility and creates an authenticated session", async () => {
  ...
  await user.type(screen.getByPlaceholderText("Re-enter your password"), "ExamplePassword1!");
